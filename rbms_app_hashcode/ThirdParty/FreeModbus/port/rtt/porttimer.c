...
static void com1_timer_timeout_ind(void* parameter);
static void com2_timer_timeout_ind(void* parameter);
...
BOOL xMBPortTimersInit(ULONG port, USHORT usTim1Timerout50us)
{
    ...
    if(port == COM2){
        stimer[1] = hTimCreate();
    	hTimSet(stimer[1], ms, &com2_timer_timeout_ind);
    }
    return TRUE;
}
