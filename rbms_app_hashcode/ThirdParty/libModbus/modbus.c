...
static int32_t User_func(const uint8_t _slave,
 ...
 uint8_t mbData[MAX_MESSAGE_LENGTH];
 uint8_t mbRdData[MAX_MESSAGE_LENGTH];
static __MB_ANSW_READY_DATA  mbAnsw;
static uint32_t mbMasterState = MBMSATER_NOT_INIT ;
uint32_t mbMasterRcvState;
uint32_t mbMasterSndState;
//static uint32_t mbMasterTmode;

static void MBMaserCallbackTimerExpired(void);

/* ----------------------------------------------------------------------------------------
    Static Function
 ----------------------------------------------------------------------------------------*/
static void mbMasterRTUStart(void)
{
    mbMasterRcvState = STATE_M_RX_INIT;
    mbMasterPortSerialEnable(TRUE, FALSE);
    mbMasterPortTimerT35Enable(); 
}

#if 0
static void mbMasterRTUStop(void)
{
    mbMasterPortSerialEnable( FALSE, FALSE );
    mbMasterPortTimersDisable(  );    
}
#endif
...
void mbMasterRTUSend(uint32_t slaveAddr, uint32_t func, uint32_t addr, uint32_t val, uint8_t *muldata )
{
    ...
    if(mbMasterRcvState == STATE_M_RX_IDLE)
    {
        ...
        q.pData = muldata;
        mbMasterFlushData();
        mbMasterSetMode(slaveAddr, func);
        mbMasterMakeQuery(&q);    
        mbMasterSndState = STATE_M_TX_XMIT;
        mbMasterPortSerialEnable(FALSE, TRUE);
    }
}

static void mbMasterRTUTransmit(void)
{
    int temp;
    ...
    case STATE_M_TX_XMIT:
        ...
        {
            mbMasterPortSerialPutByte(*(s_query.pQuery+temp));
        }
        mbMasterPortSerialSend();
        mbMasterSndState = STATE_M_TX_XFWR;
        break;
    case STATE_M_TX_XFWR:
        mbMasterPortSerialEnable(TRUE,FALSE);
        mbMasterPortTimerRspTimeoutEnable();
        break;
        
    }
}

static void mbMasterRTUReceived()
{
    char data;
...
static void mbMasterRTUExipred(void)
{
	...
	case STATE_M_RX_INIT:
		mbMasterPortEventPost(EV_MASTER_READY);
		break;
	...
	case STATE_M_RX_RCV:
		mbMasterPortEventPost(EV_MASTER_FRAME_RECEIVED);
		break;
	...
	case STATE_M_TX_XFWR:
			mbMasterSetErrorCode(MB_MASTER_ERR_TXTIMEOUT);
			mbMasterPortEventPost(EV_MASTER_ERROR_PROCESS);
		break;
		/* Function called in an illegal state. */
	default:
...
PUBLIC void initmbMasterVariables(){
	s_query.pQuery = &mbData[0];
    s_pAnsw = &mbAnsw;
    s_pAnsw->pAnsw = &mbRdData[0]; 
    s_pAnsw->answLen = 0;
}
...
{
    mbMasterPortSerialInit( NULL, baudrate, 8, 0);
	mbMasterPortSerialSetTimeout(20);
    return 0;
}
#endif    
/**
...
/**
    ...
    ******************************************************************************
*/
 PUBLIC int32_t mbMasterMakeQuery(MB_QUERY* pQueryData)
 {
    int32_t err = 0;
...
PUBLIC int32_t mbMasterProcessRcvdData(void) 
{
    ...
    switch (s_query.func)
    {
        ...
        case FC_READ_INPUT_REGISTERS:
          ...
          len -= 5;
          hSysLock();
          if(mbMasterPortGetByteOrder()== TRUE){
...
int32_t mbMasterReadValue(uint16_t *buf, int size)
{
    int i, len;
    uint8_t *ptr;
    hSysLock();
...
{
    uint32_t utime;
    mbMasterPortSerialInit(ucPort, ulBaudRate,8,0);
    if(ulBaudRate > 19200){
        utime = 5600;
    }else{
        utime = 50000;
    }
	mbMasterPortTimerInit(utime, MBMaserCallbackTimerExpired );
}
...
PUBLIC int mbMasterGetErrorCode(void)
{
    return error_code;
}
