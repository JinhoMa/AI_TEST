...
void com2CallbackFromIsr(int mode)
{
    if(mode == 0){
        pxMBFrameCBByteReceived(COM2);
    }else{
        //mbMasterCallbackTrasmitterEmpty();
    }
}
        ...
        eMBParity eParity)
{
    ...
    assert_param( COM1<=ucPORT && ucPORT<=COM2 );
    /**
    ...
    {
        case COM1:
            mb_uart_fd[COM1] = hSerOpen("COM2", ulBaudRate, com1CallbackFromIsr );
            break;
        case COM2:
            mb_uart_fd[COM2] = hSerOpen("COM3", ulBaudRate, com2CallbackFromIsr );
            break;
    }
    if(!mb_serSema){
#if RTOS        
        mb_serSema =  hOsBiSemaCreate();
        hOsSetThreadName(mdbsSlave_th, "mbport");
        mdbsSlave_th.priorty = (int32_t) H_OSTHRD_PRI_NORMAL;
        tid = hOsCreateThread( &mdbsSlave_th,   (void *)&mbSlavePortThread ,NULL);   
        hOsResumeThread(tid);
#endif        
    }
    return TRUE;
...
{
    assert_param( COM1<=port && port<=COM2 );
    if(xRxEnable)
    {
    }
    else
...
static void mbSlavePortThread(void const * parameter) {
    ...
    while (1)
    {
        ...
        if( mbTxStartEvent[COM2] == TX_START_EVENT ){
            /* execute modbus callback */
            prvvUARTTxReadyISR(COM2);
        }
        else{
            hOsBiSemaWait(mb_serSema  , timeout);
        }
    }
}
#else
